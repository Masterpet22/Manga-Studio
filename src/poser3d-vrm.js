...
let yaw=.35,pitch=.04,distance=4.5,avatarYaw=0,orbitDrag=null,ikDrag=null,currentPreset='neutral',skeletonHelper=null;
...
async function loadAvatar(url=avatarUrl){
  ...
  model=vrm.scene;avatarYaw=0;model.rotation.y=0;document.getElementById('modelYaw').value=0;scene.add(model);model.traverse(object=>{if(!object.isMesh)return;object.castShadow=true;object.receiveShadow=true;const list=Array.isArray(object.material)?object.material:[object.material];list.forEach(material=>{if(material.color&&!material.userData.originalColor)material.userData.originalColor=material.color.clone();if(material.name.includes('_CLOTH')&&!clothMaterials.includes(material))clothMaterials.push(material)})});
...
function presetPoint(values,side){const point=new THREE.Vector3(avatarCenter.x+rigSide[side]*values[0]*modelHeight,groundY+values[1]*modelHeight,avatarCenter.z+values[2]*modelHeight);if(avatarYaw)point.sub(model.position).applyAxisAngle(new THREE.Vector3(0,1,0),avatarYaw).add(model.position);return point}
...
function toAvatarSpace(point){return point.clone().sub(model.position).applyAxisAngle(new THREE.Vector3(0,1,0),-avatarYaw)}
function fromAvatarSpace(point){return point.clone().applyAxisAngle(new THREE.Vector3(0,1,0),avatarYaw).add(model.position)}
function mirroredTarget(point){const local=toAvatarSpace(point);local.x*=-1;return fromAvatarSpace(local)}
function mirrorPose(){const pairs=[['leftHand','rightHand'],['leftFoot','rightFoot']];for(const [left,right]of pairs){const lp=targets[left].position.clone(),rp=targets[right].position.clone();targets[left].position.copy(mirroredTarget(rp));targets[right].position.copy(mirroredTarget(lp))}activeBody=structuredClone(activeBody);for(const rotation of Object.values(activeBody)){rotation[1]*=-1;rotation[2]*=-1}captureTargetBases();markCustom();solveAll()}
function copyLeftToRight(){for(const [left,right]of [['leftHand','rightHand'],['leftFoot','rightFoot']])targets[right].position.copy(mirroredTarget(targets[left].position));captureTargetBases();markCustom();solveAll()}
function setModelYaw(degrees){if(!model)return;const next=THREE.MathUtils.degToRad(degrees),delta=next-avatarYaw,pivot=model.position.clone(),axis=new THREE.Vector3(0,1,0);avatarYaw=next;model.rotation.y=next;for(const target of Object.values(targets))target.position.sub(pivot).applyAxisAngle(axis,delta).add(pivot);for(const base of Object.values(targetBases))base.sub(pivot).applyAxisAngle(axis,delta).add(pivot);solveAll()}
...
document.getElementById('modelYaw').oninput=event=>setModelYaw(+event.target.value);
...
document.getElementById('resetPose3d').onclick=()=>{yaw=.35;pitch=.04;distance=Math.max(3.5,modelHeight*2.55);setModelYaw(0);document.getElementById('modelYaw').value=0;applyOutfit('original');applyPreset('neutral');cameraUpdate()};
